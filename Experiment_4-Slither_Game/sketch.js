...
Modifications: Changed the colours of the slither(snake) and grid. 
...
function draw() {
  ...
  */
  if (!gameOver) {
    let survivalTime = int((millis() - timer) / 1000); // Calculate seconds: (Current time - Start time) / 1000
...
function displayBoard(){
  for (let i=0; i<cols; i++){
    for (let j=0; j<rows; j++){
      if (board[i][j] == 0){
        fill(0); // Changed the board colour to black
      } else if(board[i][j] == -1){
        fill(153, 204, 0); // Colour for the food block
      } else if(board[i][j] == -2){
        fill(255, 204, 0); // Added a colour to power-up block
      } else if(board[i][j] == -3){
        fill(255, 0, 0); // Added a colour to scissors block 
      } else {
        fill(random(1,255), random(1,255), random(1,255),random(255));// Changed the colours to random for the slither 
      }
      rect(i*size, j*size, size, size); // displayText(i, j)
    }
  }
}
